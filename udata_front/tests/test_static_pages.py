...
from udata_front.views.gouvpt import get_pages_gh_urls, detect_pages_extension
...
class StaticPagesTest:
    ...
    def test_page_does_not_exist(self, client, rmock):
        ...
        response = client.get(url_for('gouvpt.show_page', slug='doesnotexist'))
    ...
    def test_page_error_w_cache(self, client, rmock, mocker):
        ...
        response = client.get(url_for('gouvpt.show_page', slug='cache1'))
    ...
    def test_page_md(self, client, rmock):
        ...
        rmock.head(f'{raw_url}.md', status_code=200)
        rmock.get(f'{raw_url}.md', text="""#test""")
        response = client.get(url_for('gouvpt.show_page', slug='test'))
        assert response.status_code == 200
...
""")
    ...
    def test_page_subdir(self, client, rmock):
        ...
        rmock.head(f'{raw_url}.md', status_code=200)
        rmock.get(f'{raw_url}.md', text="""#test""")
        response = client.get(url_for('gouvpt.show_page', slug='subdir/test'))
        assert response.status_code == 200
        assert b'<h1>test</h1>' in response.data
